...
class SuL_Setting {
  ...
  public function input($name, $display = NULL, $description = NULL, $default = NULL, $desType = NULL) {
    ...
    $string .= '<input class="mdui-textfield-input" id="' . $name . ' " type="text" name="' . $name . '" value="' . htmlspecialchars($userOption) . '" />';
    $string .= $description;
    $string .= '</div>';
    ...
    public function text($name, $display = NULL, $description = NULL, $default = NULL, $desType = NULL) {
    $string = '';
    if ($desType === true) {
      $description = $description . '<br/>';
    } else {
    ...
    $userOption = themeOptions($name);
    $string .= '<br><div class="mdui-textfield">';
    $string .= '<p class="mdui-textfield-input" id="' . $name . ' " type="text" name="' . $name . '" value="' . htmlspecialchars($userOption) . '" />' . $display . '</p>';
    $string .= $description;
    $string .= '</div>';
  ...
  public function select($name, $options, $display = NULL, $description = NULL, $default = NULL, $desType = NULL) {
    ...
    $string .= '<label class="mdui-textfield-label">' . $display . '</label>';
    $string .= '</div>';
    $string .= '<select class="mdui-select" id="' . $name . ' " name="' . $name . '" mdui-select="{position: \'bottom\'}">';
  ...
  public function textarea($name, $display = NULL, $description = NULL, $default = "", $rows = NULL) {
    ...
    $string .= '<div class="mdui-textfield"><label class="mdui-textfield-label">' . $display . '</label><textarea class="mdui-textfield-input" id="' . $name . ' " type="text" name="' . $name . '"' . $rows . '/>' . htmlspecialchars($userOption) . '</textarea>' . $description . '</div>';
  ...
  public function plugin($name, $description = NULL, $content = NULL) {
    ...
    if(!array_key_exists($name, $plugins)){
      $content = '检测到您还未安装“<a href="https://github.com/Mejituu/Links/releases/tag/v1.2.6">' . $name . '</a>”插件，请安装该插件，否则将无法正常运行其功能，若安装了，确保插件是启动状态';
    }
